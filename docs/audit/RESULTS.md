...
## Round 8 — Progress charts in detail, and the three mocks

### Progress charts — PASS
- "Show the numbers" toggles to "Hide the numbers" and renders a real <table>:
  Week / Listening / Reading / Writing / Speaking / Overall, with ISO week rows (2026-W31).
  This is the documented contrast relief, not decoration.
- Single-series view selects and redraws (41 svg paths: line, range band, endpoints)
- CRITERION BREAKDOWN, the documented split, verified on both sides:
  * Writing gets the radar, axes TA CC LR GRA, with FULL NAMES listed underneath because an
    axis can only carry the initialism: "Task Achievement (TA) 3.5, Coherence and Cohesion
    (CC) 4.0, Lexical Resource (LR) 4.0..."
  * Reading gets question-type accuracy instead, weakest first, with no rubric criteria:
    "matching sentence endings 0/10 · 0%, true false not given 20/78 · 26%,
     Summary completion 8/30 · 27%"

### Reading mock — PASS, sat end to end
- "Start the hour" opens the sitting at 59:54 with the conditions in the header:
  "No dictionary, no coaching, no pause, and 60 minutes including writing your answers down."
- REMOVED: Pause, Coach. KEPT: Flags. Live pacing: "0:06 elapsed · you should be on passage 1"
- The dictionary is genuinely off: double-clicking a word produces NO popover, no
  "Add to vocabulary", and no "Look up" in the selection toolbar
- Hand-in confirm: "36 of 40 questions are still blank, and a blank scores nothing while a
  guess sometimes does. Marking is immediate and the paper cannot be reopened."
- REPORT LEADS WITH PACING, then raw, then band, and gives ONE verdict not a list:
  "This was a pacing problem before it was a reading problem. 36 blanks and 0 questions over
  two minutes. Fix the clock first..." then "2/40 Raw score, the real metric" and
  "2.0 Estimated band"

### Writing mock — PASS, sat end to end
- 59:54, both tasks switchable from minute zero with live counts (Task 1 0/150, Task 2 0/250)
- The coaching machinery is structurally absent: no Phrase help, no Coach,
  spellcheck="false", data-gramm="false", data-enable-grammarly="false"
- Chart affordances kept, correctly — they are the task material, not help
- REPORT LEADS WITH TIME: "Task 2 got 0 min of the hour. It carries two thirds of the writing
  band and needs closer to forty minutes... Whatever it did not get came out of the score that
  matters most." Per-task deltas: Task 1 −19 min, Task 2 −40 min
- THE UNMARKED REASON IS THE TRUE ONE: "This answer is 0 words. Under 50 words there is not
  enough writing..." — the documented fix for a report that once blamed the 50-word floor for
  every unmarked answer

### Listening mock — server gates PASS, sitting not sat (render incomplete)
- The plan endpoint previews without opening a sitting, as documented
- "Open the paper and render its audio" DOES create the sitting (status: preparing)
- One-sitting-at-a-time enforced: a second create 409s with
  "listening mock lm_... is still open — submit it, or POST .../abandon, before starting another"
- START REFUSED UNTIL AUDIO EXISTS, and the refusal names the counts exactly as documented:
  409 "the audio for this sitting is not ready yet — 3 of 4 parts rendered. Poll GET .../sessions/... until status is 'ready'."
- PLAY REFUSED BEFORE START: 409 "listening mock lm_... is preparing — press start first"

#### Two bugs found in the listening mock pre-flight
1. THE DOCUMENTED RENDER PROGRESS IS NEVER SHOWN. FEATURES.md describes a per-part list
   ("ready · 3:41" / "rendering") and an aria-live line ("{n} of 4 parts rendered, and this one
   is 63% done"). Neither appears, in the preparing state or on reload. The server hands the UI
   exactly this data — its own 409 says "3 of 4 parts rendered" — so the information is
   available and the screen simply does not render it. A learner opening a paper sees a static
   page with no sign that anything is happening for several minutes.
2. THE RESUME BANNER LIES ABOUT THE CLOCK. For a sitting at status "preparing", whose clock has
   not started, it says "You already have a paper open. ITS CLOCK HAS BEEN RUNNING SINCE IT
   STARTED, and the recordings it has already used cannot be played again." Both halves are
   false while preparing, and the documented copy for this state is the opposite:
   "the clock has not started and will not until you press start". A learner who opened a paper
   and stepped away while it rendered is told their hour is burning.
